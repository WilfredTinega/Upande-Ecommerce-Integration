...
def get_delivery_gln_from_sales_order(sales_order):
	...
	try:
		...
		if gln:
			return gln
		safe_log(
			f"Sales Order {sales_order.name} has no custom_floriday_delivery_id", "Missing GLN", "warning"
		)
		return None
	except Exception as e:
		safe_log(f"Error getting GLN for sales order: {e!s}", "GLN Lookup Error", "error")
		return None
		...
		if response.status_code == 200:
			...
			for item in items:
				if item.get("salesOrderId") == sales_order_id:
					...
					if req_id:
						safe_log(
							f"Found fulfillmentRequestId {req_id} for salesOrderId {sales_order_id}",
							"Fulfillment Request Lookup",
							"info",
						)
						return req_id
			...
			if len(items) == 1:
				...
				if req_id:
					return req_id
			safe_log(
				f"No fulfillmentRequestId found for salesOrderId {sales_order_id}. Response: {str(data)[:300]}",
				"Fulfillment Request Lookup Warning",
				"warning",
			)
		else:
			safe_log(
				f"GET /fulfillment-requests failed: {response.status_code} {response.text[:200]}",
				"Fulfillment Request Lookup Error",
				"error",
			)
	except Exception as e:
		safe_log(f"Error fetching fulfillment requests: {e!s}", "Fulfillment Request Lookup Error", "error")
...
def build_delivery_order_index(base_url, headers):
	...
	while pages < 200:
		try:
			...
			safe_log(
				f"delivery-orders/sync error at seq {seq}: {str(e)[:120]}",
				"Floriday Delivery Order Sync",
				"error",
			)
			break
		if r.status_code != 200:
			safe_log(
				f"delivery-orders/sync HTTP {r.status_code} at seq {seq}: {r.text[:150]}",
				"Floriday Delivery Order Sync",
				"error",
			)
			break
...
def update_delivery_note_with_fulfillment(sales_order_name, fulfillment_id):
	...
	try:
		...
		safe_log(f"Could not update delivery note: {e!s}", "Delivery Note Update Error", "warning")
...
def order_fullment():
	...
	try:
		...
		for so in sales_orders:
			...
			try:
				...
				if not delivery_gln:
					...
					safe_log(
						f"Delivery order {do_entry.get('delivery_order_id')} had no GLN; using fallback {delivery_gln}",
						"Default GLN Used",
						"warning",
					)
				step(
					f"  STEP 4b: Packages = {number_of_packages}, GLN = {delivery_gln} (DO {do_entry.get('delivery_order_id')})"
				)
				...
				step(
					f"  STEP 4c: fulfillmentRequestId={fulfillment_request_id}, new fulfillmentOrderId={new_fulfillment_order_id}"
				)
				...
				error_count += 1
				results.append({"sales_order": sales_order_name, "status": "error", "message": str(e)[:300]})
		...
		frappe.log_error(
			f"Floriday Fulfillment Fatal Error: {str(e)[:150]}\n{traceback.format_exc()[:200]}",
