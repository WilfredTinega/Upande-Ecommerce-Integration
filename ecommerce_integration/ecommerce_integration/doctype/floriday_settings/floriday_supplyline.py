...
def create_supply_lines_only_from_batches():
	...
	try:
		...
		safe_log(
			f"Results: {len(successful_supply_lines)} success, {len(failed_supply_lines)} failed",
			"Floriday Complete",
		)
...
def create_single_supply_line(BASE_URL, API_KEY, ACCESS_TOKEN, batch):
	...
	try:
		...
		supply_line_payload = {
			...
			"pricePerPiece": {"currency": SUPPLY_LINE_CURRENCY, "value": float(offer_price)},
			"orderPeriod": {"startDateTime": now.isoformat(), "endDateTime": order_end.isoformat()},
			"deliveryPeriod": {"startDateTime": now.isoformat(), "endDateTime": order_end.isoformat()},
		...
		response = requests.post(supply_line_endpoint, json=supply_line_payload, headers=headers, timeout=30)
...
def get_your_floriday_batches(
	BASE_URL, API_KEY, ACCESS_TOKEN, SUPPLIER_ORG_ID, tail_only=False, tail_size=1500, return_error=False
):
	"""
...
def diagnose_floriday_batches():
	...
	try:
		...
		for field in (
			...
			count = sum(1 for b in all_batches if b.get(field) and current_date in str(b.get(field)))
